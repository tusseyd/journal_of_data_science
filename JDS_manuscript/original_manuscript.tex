...
a summary of SR counts over a five-year period from 2020 to 2024. 
...
and the presence of missing, blank, or N/A entries. 
...
Section~\ref{sec:principles} offers 
...
and a yearly record of 3.5 million SRs in 2024. It 
appears that 2025 is on track to surpass that record. 
Figure~\ref{fig:annualSRs} illustrates 
the nearly 20\% growth since 2020.
...
and was collected on 10 October 2025. This dataset is used 
to conduct analysis in areas such as data consistency, data validity, 
redundant fields, etc. Instructions 
on downloading this file from the NYC Open Data portal 
are available in the supplemental material. 
Characteristics of this five-year 311 SR dataset include:

\begin{itemize}[left=1.5em, itemsep=0.25\baselineskip]
\item The dataset is approximately 9 GB in size and contains 
more than 16 million rows, with each row representing a single 
SR. The dataset exports as a CSV file. For efficiency in the R 
programming environment, the file was converted to the RDS format.

\item Each record has 41 columns (fields) of data, with each row 
representing a single SR (e.g., complaint, issue, request). 

\item There are 252 distinct \texttt{complaint\_type}(s) with the top twenty
complaints accounting for 68\% of all SRs. 
...
of all SRs.  As noted, noise-related complaints constitute 
nearly a quarter of all complaints. Other prominent 
categories include illegal parking, heat/hot water 
...
Anomalies in the dataset are typically associated with specific city
...
Even a small number of implausible durations can shift summary statistfics and
...
For some attributes, the Data Dictionary specifies domains
...
documentation, or external datasets. The NYC Open Data 
portal supplements this documentation
...
As a result, several anomalies were found where 
...
The \texttt{community\_board} field captures one of New York City’s units
...
Such anomalies underscore the risks when categorical fields linked to
administrative structures are not validated.
...
both the extent and volume of these long post-closed updates,
...
an artificially post-closed update duration. Such behavior 
...
\end{subfigure}
% Main figure caption
\caption{Violin plots for post-closed resolution action updates and negative SR durations}
...
during processing.
When a timestamp is absent and field is converted to a datetime value,
 the typical system behavior is to assign a default value of
...
\begin{itemize}[left=1.5em, itemsep=0.25\baselineskip, align=parleft, label=\textbullet]
  ...
  likely indicating that these fields serve different purposes. 
  ...
  Including both seems redundant, especially given that NYC agency 
...
quality. Automated systems can proactively detect and possibly 
correct common issues, reducing the manual workload. Collaboration 
with city agencies and stakeholders is crucial to ensure that 
data standards, formats, and metadata meet the needs 
...
standards. Such tools could capture and display quality 
metrics of the data sets as presented
...
inspired by the potential of open data to enhance public services. Government's 
support of such initiatives, not only promotes 
data literacy, but encourages community-driven improvements to
datasets. These events strengthen public engagement and
demonstrate the societal value of open data initiatives. By 
leveraging the expertise of data scientists, statisticians, stakeholders, 
...
\section*{Supplementary Material}
The \textsc{R} source code used to produce these results, along with
additional analyses and charts are available at
\url{https://github.com/tusseyd/journal_of_data_science/tree/main}.  

Reference data files (2020--2024 NYC~311~SR data, USPS ZIP codes) are available at 
